...
function clean_file {
    ...
    # Add back the CloudFormation helper scripts; this is a workaround for
    # https://github.com/jazzband/pip-tools/issues/93
    if [[ "${FILE_PATH}" == "playbooks/roles/aws/templates/requirements.txt.j2" ]]; then
        echo "https://s3.amazonaws.com/cloudformation-examples/aws-cfn-bootstrap-latest.tar.gz" >> ${FILE_PATH}
    fi
